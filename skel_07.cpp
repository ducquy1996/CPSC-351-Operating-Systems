...
/* TODO: Now, lets read a message from the parent */
    ...
    //fprintf(stderr,"Message from parent to child: %s\n",fileNameRecv);
...
/* TODO: close the unused ends of two pipes. */
    ...
    if(close(childToParentPipe[WRITE_END])<0){
        perror("ctpWRITE close");
        exit(-1);
    }
    if(close(parentToChildPipe[READ_END])<0){
        perror("ptcREAD close");
        exit(-1);
    }
...
/* TODO: Send the string to the child */
 ...
 /* TODO: Read the string sent by the child */
    if(read(childToParentPipe[READ_END],hashValue,HASH_VALUE_LENGTH)<0){        // String stored in hashValue
        perror("ctpRead");
        exit(-1);
    }
    //fprintf(stderr,"Message from child to parent: %s\n",hashValue);
...
/* Run a program for each type of hashing algorithm hash algorithm */
    ...
    /* Fork a child process and save the id */
        ...
        /* TODO: close the unused ends of two pipes */
            ...
            // child read msg from the parent/ child send string to the parent... ctpW  ,ctpR,ptcW, ptcR
            if(close(childToParentPipe[READ_END])<0){
                perror("C ctpCloseRead");
                exit(-1);
            }
            if(close(parentToChildPipe[WRITE_END])<0){
                perror("C ptcCloseWrite");
                exit(-1);
            }
